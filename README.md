...
### 2026-06-01 (后) · 魔法阵手动施法 + 法术专属增益 + 移动端适配 🎮

上一版的"自动循环施法"上手没掌控感，所有增益又只是"加伤害/范围/减冷却"三件套，对不同类型法术显得很套路化。这次大改：

#### 🎯 手动施法
- **法术不再自动轮转**。玩家拥有 5 个法术，引擎不会替你选
- **长按开火键 = 蓄力当前选中法术**；蓄满自动释放，进入冷却
- **松开开火 = 取消蓄力**（不消耗冷却，不再误触）
- **切换法术**：桌面端 `Q` 键 / 武器按钮 / **手机端点击屏幕底部的法术轮**
- 切换瞬间会有一个高亮反馈，方便辨认当前选中

#### 🪄 每法术专属增益（共 20 个）
旧的"伤害+30% / 范围+25% / 冷却-20%"保留，但只作为 4 个**全法术整体增益**。每个法术现在还有 **2 个针对自身痛点**的专属强化：

**🔥 火系**
| 法术 | 痛点 | 增益 1 | 增益 2 |
|------|------|--------|--------|
| 流星雨 | 数量型 | 流星增援 (+2 颗 ×3) | 碎裂陨石（落地分裂 3 颗子流星） |
| 火焰新星 | 一次性瞬伤 | 二段爆裂（0.3s 后再爆一次） | 余烬地带（留下 2s 灼烧地带） |
| 熔岩飞弹 | 追踪型 | 熔岩齐射 (+2 颗 ×3) | 裂地爆炸（爆炸半径 +100% ×2） |
| 烈焰风暴 | 持续 DOT | 永焰（+2 秒持续 ×3） | 烈焰附身（跟随玩家移动） |
| 火神之锤 | 单体单次 | 余震（+1 道扩散冲击环 ×3） | 飞火溅星（召唤 4 颗溅射小流星） |

**⚡ 电系**
| 法术 | 痛点 | 增益 1 | 增益 2 |
|------|------|--------|--------|
| 闪电链 | 跳数衰减 | 链式扩展（+4 跳 ×2） | 完美导体（无伤害衰减） |
| 天雷 | 散射不稳 | 雷霆增幅（+3 道 ×2） | 蓄电核心（落点 1s 后再爆一次） |
| 静电场 | 持续/范围 | 持久磁场（+2 秒 ×3） | 电磁束缚（半径 +50% & 减速敌人） |
| 电磁轨道炮 | 单发激光 | 加宽聚束（宽度 +100% ×2） | 双管齐射（0.12s 后再射一发） |
| 电浆轰炸 | 数量型 | 电浆增量（+2 颗 ×2） | 电浆裂变（命中后分裂 4 颗小球） |

> 增益按派系过滤，选了火系不会再出现电系的卡，反之亦然。

#### 📱 移动端 UI 适配
之前的 UI 在 ≤640px 屏幕上几乎要溢出，这次做了系统性瘦身：

- **Canvas HUD**（RogueUI）通通走 `uiScale` 系数（0.65 / 0.7 / 0.8 / 1.0），所有文字/卡片/状态条/技能按钮自动缩放
- **武器/派系/Perk 选择卡**：移动端从横向 3 张改为**竖排堆叠**，单卡更短更紧凑（左大图标 + 右文字）
- **Boss 血条**：移动端缩窄为 `screenW - 180`，避免和两侧徽章重叠
- **Perks 面板**：移动端每行 4 个图标（桌面 6 个）
- **法术轮**：5 颗圆形按钮（移动端 44×44 / 桌面 56×56），底部居中
- **PLAYING 模式按钮**：技能按钮 56→68px、武器按钮 88→112px 都加了 `sm:` 断点
- **顶栏 ROGUE / EXIT 徽章**：缩小到移动端可读且不挤压

#### 🛠️ 实现细节
- `MagicCircle` 新增 `currentSpellIndex / switchToNext / setSpell / casting / getCastProgress`
- 蓄力进度 = `elapsed / (castTime × circleCastSpeedMul)`，松开瞬间重置
- 新增 `RogueEngine.getMagicState()` API，把 5 个法术的 `cooldown / cooldownMax / castTime / casting.progress` 暴露给 React HUD
- React `SpellWheel` 50ms 轮询绘制，SVG 双层环（冷却环灰填 + 蓄力白色亮线）
- 在 `RogueModifiers` 加入 `spellPerks: Record<MagicSkillId, {[key]: number}>`，每个 perk 映射到 `(skillId, key)` 然后在 `MagicCircle.fire()` 内 `spellPerk(id, key)` 读取层数
- 多个特效实体（`MagicMeteor / MagicHomingOrb / MagicAuraField`）加了 `splitCount / steerStrength / splitOnHit / slowFactor` 字段对接专属增益
- 新增 `MagicOrbLink` 实体——电浆球之间的动态电链视觉
...
| 🟧 | **FLAK**（高射炮） | 定时空爆 + 放射状碎片 |
| 🟢 | **HELIX**（螺旋光子） | 双股正弦弹幕 |
| 🟪 | **MAGIC CIRCLE**（魔法阵·肉鸽） | 5 招独立法术手动切换，长按蓄力法阵释放 |
...
- **3 种初始武器**：机枪 / 激光 / 魔法阵
- **魔法阵双流派 × 各 5 招法术**：🔥 火系（流星雨 / 火焰新星 / 熔岩飞弹 / 烈焰风暴 / 火神之锤）/ ⚡ 电系（闪电链 / 天雷 / 静电场 / 电磁轨道炮 / 电浆轰炸）—— **手动切换** + 长按蓄力释放，每招独立冷却 / 法阵图案
- **40+ 种增益卡**：通用 / 武器专属 / 主动技能解锁 / **每法术专属（10 法术 × 2 个痛点强化）** / 法阵全局四类（冷却·伤害·范围·蓄力）
- **每层 3 选 1**，构筑差异化（不会出无效卡，例如选了火系不会再出电系卡）
- **Boss 血量随层数递增**，无限挑战
